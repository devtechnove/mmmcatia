...
@section('breadcrumb','OFRENDAS')
@section('content')
<div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0">Ofrendas</h2>
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ url('/home') }}">Inicio</a>
                        </li>
                        <li class="breadcrumb-item"><a >Datos de tesorería</a>
                        </li>
                        <li class="breadcrumb-item active"> Listado de las ofrendas recibidas
                        </li>
                    </ol>
                </div>
            </div>
    ...
    <div class="col-lg-12">
      <div class="card card-line-primary">
            <div class="card-header">
              <h4 class="card-title">Listado de ofrendas recibidas</h4>
            </div>
            <div class="card-body">
                ...
                <table id="tableExport" class="display table table-striped table-hover" >
                  ...
                  <tbody>
                    @foreach ($academias as $element)
                      <tr>
                        <td>{{ $element->id }}</td>
                        <td>{{ $element->diaservicio->dia_servicio }} - {{ $element->diaservicio->name }}</td>
                        <td>{{ $element->total_pagar }}$</td>
                        <td>  {{ \Carbon\Carbon::parse($element->fecha)->format('d/m/Y') }} {{  $element->hora }}</td>
                        @if ($element->status == 1)
                          <td>
                             <span class="badge green text-white">Recibido</span>
                          </td>
                          @else
                          <td> <span class="badge red text-white">Sin recibir</span></td>
                        @endif
                        <td>
                       ...
                       @include('admin.ofrendas.partials.modal.edit')
                    @endforeach
                  </tbody>
                </table>
            </div>
  ...
  @include('admin.ofrendas.partials.modal.create')
